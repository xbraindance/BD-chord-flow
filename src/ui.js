...
import { openTextEntry, closeTextEntry, isTextEntryActive, handleTextEntryMidi, drawTextEntry, tickTextEntry } from '/data/UserData/schwung/shared/text_entry.mjs';
...
// ─── Edit rows ────────────────────────────────────────────────────────────────
const EDIT_KEYS   = ['pad','root','chord_type','inversion','bass','pad_octave','strum','strum_dir','articulation','reverse_art','global_octave','global_transpose','reset_patch','save'];
const EDIT_LABELS = ['Pad','Root','Chord Type','Inversion','Bass','Pad Oct','Strum','Strum Dir','Articulation','Reverse Art','Global Oct','Global Trans','Reset Patch','Save'];
const BUILTIN_CHORD_TYPES = [
...
const EDIT_ENUMS  = {
    ...
    chord_type:   [...BUILTIN_CHORD_TYPES],
...
// ─── Knob mapping (K1-K8, CC 71-78 → always-active in edit screen) ──────────
const KNOB_MAP = [
    { key: 'root',         options: EDIT_ENUMS.root,         type: 'enum' },         // K1
    { key: 'chord_type',   options: EDIT_ENUMS.chord_type,   type: 'enum' },         // K2
    { key: 'inversion',    options: EDIT_ENUMS.inversion,    type: 'enum' },         // K3
    { key: 'bass',         options: EDIT_ENUMS.bass,         type: 'enum' },         // K4
    { key: 'pad_octave',   min: -6,  max: 6,   step: 1,     type: 'int' },          // K5
    { key: 'strum',        min: 0,   max: 100, step: 5, fineStep: 1, type: 'int' }, // K6
    { key: 'strum_dir',    options: EDIT_ENUMS.strum_dir,    type: 'enum' },         // K7
    { key: 'articulation', options: EDIT_ENUMS.articulation, type: 'enum' },         // K8
];

// ─── Per-set state persistence ────────────────────────────────────────────────
const SET_STATE_DIR   = "/data/UserData/move-anything/set_state";
const ACTIVE_SET_FILE = "/data/UserData/schwung/active_set.txt";
const SET_CHECK_INTERVAL = 30; // ticks between UUID polls

let activeSetUuid  = "";
let setCheckTicks  = 0;

function getActiveSetUuid() {
    const raw = host_read_file(ACTIVE_SET_FILE);
    if (!raw) return "";
    const line = raw.split("\n")[0];
    return line ? line.trim() : "";
}

function setStatePath(uuid) {
    if (!uuid) return null;
    return SET_STATE_DIR + "/" + uuid + "/chord_flow.json";
}

function saveSetState() {
    const path = setStatePath(activeSetUuid);
    if (!path) return;
    const state = dspGet('state');
    if (state) host_write_file(path, state);
}

function loadSetState(uuid) {
    const path = setStatePath(uuid);
    if (!path) return;
...
// Preset name scrolling
let scrollTick = 0;
let scrollOffset = 0;
const SCROLL_SPEED = 15;           // ticks between character shifts
const SCROLL_MIN_LENGTH = 18;      // only scroll names longer than this (max displayable width)
const SCROLL_PADDING = 3;          // spaces between loop repetitions
...
// ─── Text Scrolling ───────────────────────────────────────────────────────────
function scrollText(text, maxWidth, scrollPos) {
    if (text.length * 6 <= maxWidth) {
        // Text fits — no scrolling needed
        return { displayText: text, fits: true };
    }

    // Text is too long — scroll it
    const maxChars = Math.floor(maxWidth / 6); // How many chars fit on screen
    const totalChars = text.length;
    const loopLength = totalChars + SCROLL_PADDING; // Add padding between loops
    const normalizedPos = scrollPos % loopLength;

    // Grab characters from position, wrapping around
    let displayText = '';
    for (let i = 0; i < maxChars; i++) {
        const charIdx = (normalizedPos + i) % loopLength;
        if (charIdx < totalChars) {
            displayText += text[charIdx];
        } else {
            displayText += ' '; // Padding between loops
        }
    }

    return { displayText, fits: false };
}
...
function refreshCustomChords() {
    const count = parseInt(dspGet('custom_chord_count')) || 0;
    // Debug: log module_dir and custom_chords path
    const moduleDirDebug = dspGet('_debug_module_dir') || '(empty)';
    const customPathDebug = dspGet('_debug_custom_chords_path') || '(empty)';
    console.log('Custom chords: count=' + count + ', module_dir=' + moduleDirDebug + ', path=' + customPathDebug);

    if (count > 0) {
        const names = dspGet('custom_chord_names');
        const customs = names ? names.split(',') : [];
        console.log('Custom chord names: ' + (names || '(empty)'));
        EDIT_ENUMS.chord_type = [...BUILTIN_CHORD_TYPES, ...customs];
    } else {
        EDIT_ENUMS.chord_type = [...BUILTIN_CHORD_TYPES];
    }
    KNOB_MAP[1].options = EDIT_ENUMS.chord_type;
}
...
function selectPreset(idx) {
    ...
    dspSet('preset', idx);
    refreshPreset();
    scrollOffset = 0;  // Reset scroll position when switching presets
    scrollTick = 0;
    saveSetState();
    needsRedraw = true;
}
...
function triggerSave() {
    console.log('triggerSave called');
    const fallbackName = `Preset ${Math.max(1, presetCount + 1)}`;
    console.log('Opening text entry with name:', fallbackName);
    openTextEntry({
        title: 'Save Preset',
...
function handleKnob(cc, value) {
    ...
    const knobIdx = cc - CC_KNOB1;
    if (knobIdx < 0 || knobIdx >= KNOB_MAP.length) return;

    const delta = decodeDelta(value);
    if (delta === 0) return;

    const knobDef = KNOB_MAP[knobIdx];
    const key = knobDef.key;

    if (knobDef.type === 'enum') {
        const cur = editVals[key] || knobDef.options[0];
        let idx = knobDef.options.indexOf(cur);
        if (idx < 0) idx = 0;
        idx += delta > 0 ? 1 : -1;
        idx = ((idx % knobDef.options.length) + knobDef.options.length) % knobDef.options.length;
        dspSet(key, knobDef.options[idx]);
        editVals[key] = knobDef.options[idx];
    } else {
        const step = shiftHeld ? (knobDef.fineStep || knobDef.step) : knobDef.step;
        const cur = parseInt(editVals[key] || '0') || 0;
        const next = clamp(cur + step * delta, knobDef.min, knobDef.max);
        dspSet(key, next);
        editVals[key] = String(next);
    }
    needsRedraw = true;
}
...
function drawBrowserScreen() {
    ...
    if (presetCount > 0) {
        ...
        // Render preset name with scrolling for long names
        const displayName = presetName || '---';
        const maxScrollWidth = 114;  // Fits within red boundary lines (roughly 4 to 120)

        if (displayName.length > SCROLL_MIN_LENGTH) {
            // Use scrolling for long names — position at left boundary
            const nameScroll = scrollText(displayName, maxScrollWidth, scrollOffset);
            print(10, CENTER_Y + 4, nameScroll.displayText, 1);
        } else {
            // Center short names
            const name  = trunc(displayName, 21);
...
function drawEditScreen() {
    ...
    if (editValueMode) {
        ...
        print(68, FOOTER_Y, 'Clk: done', 1);
    } else {
        print(1,  FOOTER_Y, 'Jog: row', 1);
        print(68, FOOTER_Y, 'Clk: edit', 1);
    }
    host_flush_display();
    needsRedraw = false;
}
...
globalThis.init = function() {
    ...
    // Per-set preset recall: load saved state for the current set
    activeSetUuid = getActiveSetUuid();
    setCheckTicks = 0;
    if (activeSetUuid) loadSetState(activeSetUuid);
};
...
globalThis.tick = function() {
    ...
    // Update scroll animation for preset name
    scrollTick += 1;
    if (scrollTick >= SCROLL_SPEED) {
        scrollTick = 0;
        scrollOffset += 1;
        if (screen === 'browser') {
            needsRedraw = true;
    ...
    // Poll for set changes and restore per-set preset
    setCheckTicks += 1;
    if (setCheckTicks >= SET_CHECK_INTERVAL) {
        setCheckTicks = 0;
        const uuid = getActiveSetUuid();
        if (uuid && uuid !== activeSetUuid) {
            saveSetState();
            activeSetUuid = uuid;
            loadSetState(uuid);
        }
...
globalThis.onMidiMessageInternal = function(data) {
    ...
    // Knobs 1-8: always-active param editors in edit screen
    if (status === 0xB0 && d1 >= CC_KNOB1 && d1 <= CC_KNOB8) {
        handleKnob(d1, d2);
        return;
    }
